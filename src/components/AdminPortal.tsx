...
// --- CONFIGURATION ---
const CATEGORIES = [
  "Daily Specials", // Will always appear top
  "Chai", 
  "Coffee", 
  "Milks", 
  "Immunity Boosters", 
...
interface AdminPortalProps {
  ...
  onUpdateMenu: () => void; // Used for background sync
  onBack: () => void;
  onResetSystem: () => void;
}
...
const ToggleSwitch = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <button 
    type="button" // Important: Prevent form submission refresh
...
const AdminPortal = ({ 
  ...
  // Local state for Menu to prevent page flicker (Optimistic UI)
  const [localMenuItems, setLocalMenuItems] = useState(initialMenuItems);

  // Sync props to local state when data is refetched
  useEffect(() => {
  ...
  const handleAddItem = async () => {
    ...
    const tempId = Math.random().toString(); // Temporary ID until DB confirms
    setLocalMenuItems(prev => [{...itemPayload, id: tempId}, ...prev]);
  ...
  // 3. Toggle Sugar Free Capability (Instant)
  const toggleSugarCapability = async (id: string, currentStatus: boolean) => {
    // 1. Update UI Immediately
    setLocalMenuItems(prev => prev.map(item => 
      item.id === id ? { ...item, is_sugar_free_available: !currentStatus } : item
    ));
    ...
    // No toast needed for this interaction to keep it fast
  };
  ...
  const deleteMenuItem = async (id: string) => {
    ...
    setLocalMenuItems(prev => prev.filter(i => i.id !== id)); // Remove from UI
  ...
  // --- SORTING LOGIC: DAILY SPECIALS FIRST ---
  const processedMenuItems = useMemo(() => {
    ...
    // 2. Sort (Specials First, then Alphabetical)
        ...
        {activeTab === 'orders' ? (
          <>
                    ...
                    >
                      ...
                      <div className="pl-2">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {order.status === 'sent' && (
                            <button onClick={() => onUpdateStatus(order.id, 'preparing')} className="col-span-2 bg-amber-400 hover:bg-amber-500 text-amber-950 py-3.5 rounded-xl font-black flex items-center justify-center gap-2 transition-all shadow-lg shadow-amber-400/20 active:scale-[0.98]">
                              <ChefHat className="w-5 h-5" /> START COOKING
                            </button>
                          )}
                          {order.status === 'preparing' && (
                            <button onClick={() => onUpdateStatus(order.id, 'ready')} className="col-span-2 bg-emerald-500 hover:bg-emerald-600 text-white py-3.5 rounded-xl font-black flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98]">
                              <CheckCircle2 className="w-5 h-5" /> MARK READY
                            </button>
                          )}
                          {order.status === 'ready' && (
                            <button onClick={() => onUpdateStatus(order.id, 'delivered')} className="col-span-2 bg-slate-900 text-white py-3.5 rounded-xl font-black flex items-center justify-center gap-2 transition-all shadow-lg shadow-slate-900/20 active:scale-[0.98]">
                              COMPLETE ORDER
                            </button>
                          )}
                          <button onClick={() => setOrderToCancel(order.id)} className="col-span-2 mt-1 text-[10px] font-bold text-slate-300 hover:text-red-400 flex items-center justify-center gap-1 py-2 transition-colors">
            ...
            <div className="fixed bottom-6 left-0 right-0 px-4 flex justify-center z-40 pointer-events-none">
              <button onClick={() => setShowEndDayReport(true)} className="pointer-events-auto bg-white/90 backdrop-blur-md border border-slate-200 shadow-2xl px-6 py-3 rounded-full flex items-center gap-3 text-slate-600 font-bold hover:scale-105 transition-transform">
                <div className="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center text-white"><BarChart3 className="w-4 h-4" /></div> End Day Report
              </button>
            </div>
          </>
        ) : (
              ...
              {processedMenuItems.length === 0 ? (
                ...
                processedMenuItems.map((item: any) => (
                  <motion.div 
                    ...
                    key={item.id} 
                    className={cn(
                      "bg-white p-4 rounded-2xl shadow-sm border flex justify-between items-center transition-colors group relative overflow-hidden",
                      item.available ? "border-slate-100" : "border-slate-100 bg-slate-50/50",
                      item.category === 'Daily Specials' && "border-amber-200 bg-amber-50/30"
                    )}
                  >
                    {item.category === 'Daily Specials' && (
                       <div className="absolute top-0 left-0 bg-amber-400 text-amber-900 text-[8px] font-black px-2 py-0.5 rounded-br-lg">
                         SPECIAL
                       </div>
                    )}
                    ...
                    <div className="flex items-center gap-4">
                      <div className={cn(
                        ...
                        {item.category === 'Daily Specials' ? <Sparkles className="w-6 h-6 text-amber-500" /> : item.name.charAt(0)}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <button 
                          type="button"
                          onClick={(e) => { e.stopPropagation(); toggleSugarCapability(item.id, item.is_sugar_free_available); }}
                          className={cn(
      ...
      <AnimatePresence>
        {showEndDayReport && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setShowEndDayReport(false)} />
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl relative z-10">
              <div className="bg-slate-900 p-8 text-white relative overflow-hidden">
                ...
                <div className="mt-8 grid grid-cols-2 gap-4">
                  <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/5"><p className="text-xs font-bold text-slate-400 uppercase">Total Sales</p><p className="text-2xl font-black text-emerald-400 mt-1">₹{endDayStats.totalRevenue}</p></div>
                  <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-md border border-white/5"><p className="text-xs font-bold text-slate-400 uppercase">Total Orders</p><p className="text-2xl font-black text-white mt-1">{endDayStats.totalOrders}</p></div>
                </div>
              </div>
              <div className="p-8">
                <div className="mb-6"><h3 className="text-sm font-black text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-amber-500" /> Best Seller</h3><div className="bg-amber-50 p-4 rounded-2xl flex items-center justify-between border border-amber-100"><span className="font-bold text-amber-900">{endDayStats.topItem.name}</span><span className="font-black text-amber-600 bg-white px-3 py-1 rounded-lg shadow-sm border border-amber-100">{endDayStats.topItem.count} Sold</span></div></div>
                <div className="max-h-[200px] overflow-y-auto pr-2 space-y-3 custom-scrollbar">
                  {endDayStats.itemBreakdown.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center text-sm p-3 hover:bg-slate-50 rounded-xl transition-colors"><span className="font-medium text-slate-600">{item.name}</span><span className="font-bold text-slate-900">{item.count}</span></div>
                  ))}
                </div>
                <button onClick={() => { if(confirm("Archive all orders for today?")) { onResetSystem(); setShowEndDayReport(false); } }} className="w-full mt-8 py-4 bg-slate-100 text-slate-900 font-black rounded-2xl hover:bg-slate-900 hover:text-white transition-all flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4" /> CLOSE DAY & RESET</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <AnimatePresence>
        {orderToCancel && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl relative z-10 text-center">
              ...
              <p className="text-sm font-medium text-slate-500 mb-8 px-4">This action cannot be undone.</p>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setOrderToCancel(null)} className="py-3 rounded-xl font-bold text-slate-600 hover:bg-slate-50 transition-colors">Keep Order</button>
                <button onClick={confirmCancelOrder} className="py-3 rounded-xl font-bold bg-red-500 text-white shadow-lg shadow-red-500/30 hover:bg-red-600 hover:scale-[1.02] transition-all">Yes, Cancel It</button>
              </div>
            </motion.div>
          </div>
        )}
